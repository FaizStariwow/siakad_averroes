...
// Fungsi untuk menyimpan opsi yang dipilih ke local storage
function simpanOpsiKeLocalStorage(key, value) {
  try {
...
// Fungsi untuk mengambil opsi dari local storage
function ambilOpsiDariLocalStorage(key) {
  try {
    return localStorage.getItem(key);
  } catch (error) {
    console.error("Gagal mengambil dari local storage:", error);
    return null;
  }
}

// Fungsi untuk menghapus opsi dari local storage
function hapusOpsiDariLocalStorage(key) {
  try {
    localStorage.removeItem(key);
  } catch (error) {
    console.error("Gagal menghapus dari local storage:", error);
  }
}
...
function sidebarColor(element) {
  ...
  gradientClasses.forEach((gradient) => {
    if (parent.classList.contains(gradient)) {
      parent.classList.remove(gradient);
    }
  });
  ...
  // Save the sidebar color to local storage
  simpanOpsiKeLocalStorage("sidebarColor", color);
}
...
// Retrieve and apply the saved color when the page loads
document.addEventListener("DOMContentLoaded", function () {
  var savedColor = ambilOpsiDariLocalStorage("sidebarColor");
  if (savedColor) {
    var savedElement = document.querySelector(`[data-color="${savedColor}"]`);
    if (savedElement) {
      sidebarColor(savedElement);
    }
  }
});
...
// Contoh penggunaan:
// simpanOpsiKeLocalStorage('tema', 'gelap');
// const tema = ambilOpsiDariLocalStorage('tema');
// hapusOpsiDariLocalStorage('tema');
